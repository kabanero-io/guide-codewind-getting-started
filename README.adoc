...
:layout: guide
:duration: TBD
:description: To explain how Codewind provides tools to help build high quality cloud native applications regardless of which IDE or language you use. 
...
Why VS Code? You can use Codewind for Visual Studio Code to develop and debug your containerizedprojects from within VS Code.
...
. Start to type *Codewind* in the filter field or locate and expand the *Codewind* entry in the list.
...
. Right-click your project in the *Codewind Explorer* view and select *Build.* 
. Wait for the project state to return to *Running* or *Debugging* in the *Codewind Explorer* view and then test your changes.
...
. Active kubectl context to the cluster

Install:

. Determine your ingress domain. It should be of the form .nip.io.
...
. Run ./install.sh to deploy Eclipse Che
...
. Enter the `cd `command to go to the `deploy/openshift` directory
. Deploy Che with, ./deploy_che.sh
. To create a cluster role with the required permission, start from the Codewind Che plug-in repository and run the command, kubectl apply -f setup/install_che/codewind-clusterrole.yaml
. From the Codewind Che plug-in repository run the command, kubectl apply -f setup/install_che/codewind-rolebinding.yaml 
...
. To enable privileged containers, enter, oc adm policy add-scc-to-group privileged system:serviceaccounts:eclipse-che 
. To enable containers to run as root, enter, oc adm policy add-scc-to-group anyuid system:serviceaccounts:eclipse-che 
...
. Clone the [Codewind Che plug-in repositoryhttps://github.com/eclipse/codewind-che-plugin).
. Make your modifications as you see fit. 
. The devfile.yaml and meta.yaml files need to be hosted in a location that Che can access, such as Github. 
. In devfile.yaml, modify the codewind-sidecar and codewind-theia components so that their ID formats match. To host the meta.yaml files, see: 
...
. https://github.com/kabanero-io/codewind-templates/tree/master/devfiles

Checking for the Codewind pod:

. If you are using the Terminal, switch to use the workspace namespace. You can check for the namespace with kubectl get ns.
. Ensure the projects are cloned into the workspace. You might need to refresh the browser to trigger the clone.
